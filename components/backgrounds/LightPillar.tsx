...
export function LightPillar({
  ...
  useEffect(() => {
    ...
    renderer.setPixelRatio(settings.pixelRatio * Math.min(window.devicePixelRatio || 1, 2));
  ...
  if (!webGLSupported) {
    return (
      <div className={`${styles.lightPillarFallback} ${className}`} style={{ mixBlendMode }} aria-hidden="true">
        WebGL not supported
      </div>
    );
  }
  ...
  return (
    <div
      ref={containerRef}
      className={`${styles.lightPillarContainer} ${className}`}
      style={{ mixBlendMode }}
      aria-hidden="true"
    />
  );
